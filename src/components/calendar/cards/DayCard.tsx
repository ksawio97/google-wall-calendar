    ...
    return (
        <div className="bg-gray-500 flex flex-col text-left rounded-md m-1 divide-y-4 divide-opacity-90 gap-2 divide-slate-700">
            <div className="flex flex-col p-4 gap-1">
                <p className="text-slate-900 text-lg">{day.toLocaleDateString('en-US', { weekday: 'short' })}</p>
                <p className="font-bold text-white text-3xl">{day.getDate()}</p>
            </div>
            { events.length > 0 ?
            <ul className="list-disc list-inside px-1 py-4 flex flex-col gap-1">
